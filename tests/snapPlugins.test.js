...
test('factories must be a non-empty array', () => {
  expect(() => snapPlugins([])).toThrow(/non-empty/);
  expect(() => snapPlugins('nope')).toThrow(/non-empty/);
});

test('parent mode returns the plugins plus a coordinator (buildCmd optional)', () => {
  const out = snapPlugins([fakePlugin]); // defaults to `npm run build`
